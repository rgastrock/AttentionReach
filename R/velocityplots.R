...
getVelocityProfile <- function(isi){
  ...
  for (trialno in c(0:11)){
    if(trialno %in% c(0:9)){
      dat <- read.csv(sprintf('data/p000/p000-reach-b02-t0%d.csv', trialno))
    } else{
      dat <- read.csv(sprintf('data/p000/p000-reach-b02-t%d.csv', trialno))
    }
    ...
    butterfilter <- butter(1, 3/120, type = 'low')
...
getConstantSample <- function(df, isi=8){
